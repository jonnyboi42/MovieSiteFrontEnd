...
const PurchaseTickets = () => {
  // Grab information from Redux Slice
  ...
  useEffect(() => {
    const fetchMovieImg = async () => {
      try {
        const response = await fetch(`http://localhost:3000/${movieLocation.toLowerCase()}/movie/${movieID}`);
        const data = await response.json();
        setMovieImg(data); // Update state with fetched data
      } catch (error) {
          ...
          <Card style={{ width: '30rem' }}>
            <Card.Body>
              <Card.Text className="card-text-with-bg">
                {/* Movie image with blur and overlay */}
                {movieImg?.src?.[1] && (
                  <div
                    className="card-image-blur"
                    style={{
                      backgroundImage: `url(${movieImg.src[1]})`,
                    }}
                  ></div>
                )}
